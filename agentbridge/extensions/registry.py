    ...
    return [
        ...
        ExtensionProfile(
            ...
            notes=[
                ...
                "Structured output, native handoff/guardrail/MCP/run options, approval interruption summaries, app-owned approval stores, backend-neutral ApprovalQueue resume payload helpers, guardrail diagnostics, tracing summaries, and best-effort events are covered by the plugin.",
                "Approval UIs, native SDK resume execution, and latest 0.22.x verification remain blocked or extension-level until dependency conflicts are resolved.",
            ],
        ),
        ExtensionProfile(
            ...
            notes=[
                ...
                "Structured output is validated through ADK output_schema plus AgentBridge typed validation; run diagnostics summarize sessions, services, opt-in service snapshots, event history, transfer-to-agent events, eval labels/runner bindings, and structured deployment metadata.",
                "Eval execution and deployment publishing remain extension-level.",
            ],
        ),
        ExtensionProfile(
            framework="strands",
