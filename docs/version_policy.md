...
- OpenAI Agents `0.20.x` support validates structured output through native SDK `output_type`/typed `final_output`, forwards native Agent options such as handoffs, MCP servers/config, prompts, model settings, hooks, tool-use behavior, and input/output guardrails, plus Runner options such as context, max turns, run config, previous response IDs, sessions, and conversation IDs. Approval interruptions, app-owned approval stores, backend-neutral `ApprovalQueue` resume payload helpers, resumable-state hints, handoff items, and guardrail diagnostics are normalized into result metadata/events; approval UIs and native SDK resume execution remain extension-level.
...
- Google ADK support validates structured output through ADK `output_schema` plus AgentBridge typed validation, forwards native Agent options such as descriptions, global/static instructions, schemas, generation config, mode, transfer controls, planners, code executors, retry/timeout settings, lifecycle callbacks, and sub-agents, plus Runner options such as session/memory/artifact/credential services and plugins. Run diagnostics preserve event history counts, session/service bindings, transfer-to-agent targets, opt-in session/memory/artifact service snapshots, eval labels/runner bindings, and structured deployment metadata; eval execution and deployment publishing remain extension-level.
- Strands `1.55.1` support forwards native `Agent` options such as hooks, plugins, interventions, session/memory managers, context managers, retry strategy, checkpointing, sandbox, storage, and background tasks through `StrandsExtension`. Native MCP client/tool-provider objects are appended to the Strands `tools` list and covered by a local MCP stdio server/client fixture; native intervention actions and guardrail traces are fixture-covered while JSON-safe summaries/events are normalized. Plain string MCP labels and deployment targets remain extension metadata until live AgentCore/AWS execution tests are added.
- LangGraph support validates graph execution, ToolSpec calls, deterministic structured output, conditional routing, checkpoint interrupt reporting, compiled-runtime resume, and run diagnostics.
- LangChain `1.4.0` support validates structured output through native `response_format`/`structured_response`, richer stream event normalization, run diagnostics, and forwards native `create_agent` options such as checkpointers, stores, interrupts, cache, schemas, transformers, middleware, and debug when supplied through `LangChainExtension`. AgentBridge records memory/retriever hints, includes an offline native retriever/checkpointer/store example, and provides a gated LangSmith smoke test for configured environments, but does not claim portable memory semantics across frameworks.
