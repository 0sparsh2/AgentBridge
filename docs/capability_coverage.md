...
- `agentbridge-openai-agents`: OpenAI Agents SDK handoffs, guardrails, approval interruption diagnostics, app-owned approval stores, backend-neutral `ApprovalQueue` resume payload helpers, tracing, and runner/result normalization.
- `agentbridge-strands`: Strands Agent/tool mapping, MCP clients with local stdio fixture coverage, native intervention/guardrail trace fixture coverage, hook/intervention/guardrail lifecycle events, structured output, run diagnostics, tracing, and structured AWS/AgentCore deployment metadata.
...
- `agentbridge-google-adk`: ADK sessions, opt-in memory/artifact service snapshots, sub-agent transfer diagnostics, eval labels/runner bindings, run diagnostics, and structured deployment metadata.
